    ...
    * */
    ...
    var ibGeoRoute = function (options) {
        ...
        this._getClosestPoint = function (point, level) { //Ищет в массиве точек и возвращает объект содержащий ближайшую точку к заданной
            ...
            var links = this._options.rg.links;
            ...
            function hasLink(aPoint, bPoint) {
                var link = null;
                for (var i = 0; i < links.length; i++) {
                    link = links[i];

                    if ((link.source === aPoint.id && link.target === bPoint.id) || (link.source === bPoint.id && link.target === aPoint.id)) {
                        return true;
                    }
                }
                return false;
            ...
            var pointB = {};
            pointB.id = nodes[0].id;
            pointB.x = parseFloat(nodes[0].x);
            pointB.y = parseFloat(nodes[0].y);

            for (var i = 1; i < nodes.length; i++) {
                if (hasLink(pointB, nodes[i])) {
                    pointA.id = nodes[i].id;
                    pointA.x = parseFloat(nodes[i].x);
                    pointA.y = parseFloat(nodes[i].y);
                    break;
                }
            }
    ...
    var ibGeoMap = function (options) {
        ...
        this.options.onClick = function (e, feature) {
            ...
            if (typeof JSInterface !== 'undefined') {
                ...
                window.location.replace('#response="'+JSON.stringify(response)+'"');
        ...
        this._getStyleFromFeature = function (feature) { //Функция построения объекта стиля по тэгам
            ...
            if (tags.PDF_lineColor || tags.lineColor) {
                fstyle.color = tags.PDF_lineColor || tags.lineColor;
            }

            if (tags.lineOpacity !== undefined) {
                fstyle.opacity = parseInt(tags.lineOpacity);
            }

            if (tags.lineWeight !== undefined) {
                fstyle.weight = parseInt(tags.lineWeight);
            }

            if (tags.PDF_fillColor || tags.fillColor) {
                fstyle.fillColor = tags.PDF_fillColor || tags.fillColor;
                fstyle.fill = true;
            }

            if (tags.fillOpacity !== undefined) {
                fstyle.fillOpacity = parseInt(tags.fillOpacity)+'';
            }

            if (tags.dash) {
                fstyle.dashArray = tags.dash;
            }
            return fstyle;
        };
        ...
        this._coordToLatLng = function (coords) {
            return L.Projection.Mercator.unproject({x:coords[1],y:coords[0],z:coords[2]});
        };
        ...
        this.init = function (CanvasData, POIData, callback) { //Конструктор объекта
            if (this.isInitialized()) {
                console.log('Allready initilized. Please destroy me first.');
                return;
            }

            this.indoorLayer = new L.Indoor(CanvasData, {
                higlightablePOITag: this.options.higlightablePOITag || 'zone_id',
                coordsToLatLng: this._coordToLatLng,
                getFeatureLevel: function (feature) {
                    if (!(feature.properties.relations instanceof Array) && feature.properties.relations.length === 0)
                        return null;

                    var relations = feature.properties.relations;
                    var level = {};
                    relations.forEach(function (relation) {
                        if (relation.reltags.type && relation.reltags.type === 'level' && relation.reltags.value !== undefined) {
                            level.value = relation.reltags.value;
                            level.name = (relation.reltags.name) ? relation.reltags.name : relation.reltags.value;
                        }
                    });
                    return (Object.keys(level).length === 0) ? null : level;
                },
                ...
                getStyleFromFeature: self._getStyleFromFeature,
        ...
        this.highligthPOI = function (poi_id, style, resetHilghlights) { //Функция подсветки очерченой зоны
            ...
            var poiGroup = this.indoorLayer._feateresGroup.highlightablePOI[poi_id];
            poiGroup.forEach(function (poi) {
                poi.layer.setStyle(style);
            });
            return true;
        };

        this.resetPOIHighlight = function () {
            var poiGroups = Object.keys(this.indoorLayer._feateresGroup.highlightablePOI);
            poiGroups.forEach(function(poiGroup){
                var group = self.indoorLayer._feateresGroup.highlightablePOI[poiGroup];
                group.forEach(function(poi){
                    poi.layer.setStyle(poi.defaultstyle);
                });
            });
        };
    ...
    window.initMap = function (options) {
        ...
        window.ibgeomap = new ibGeoMap({
            ...
            higlightablePOITag: opts.higlightableTag || 'zone',
    ...
    window.findRoute = function (fromPoint, toPoint, style) { //Функция поиска и отрисовки маршрута из точики from в точку to
        ...
        if (!window.indoorRouteGraph.loaded) {
            console.log('Routes: Not Initialized');
            return;
        }
    ...
    window.isTerminalExist = function () {
        ...
        var terminal = ibgeomap.isTerminalExist();
        var coords = null;
        if (window.TerminalListener && typeof window.TerminalListener.isTerminalExist === 'function') {
            if (terminal !== null) {
                coords = terminal.getCoords();
                window.TerminalListener.isTerminalExist(true, JSON.stringify({
                    x: coords.x,
                    y: coord.y,
                    l: coords.level
                }));
            } else {
                window.TerminalListener.isTerminalExist(false, "");
            }
        } else {
            return terminal;
        }
